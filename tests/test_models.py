...
def test_daily_mean_floats():
    """Test that the mean function works for an array with floating point numbers."""
    from inflammation.models import daily_mean

    test_input = np.array([[1.2, 2.1],
                           [5.9, 6.0],
                           [3.4, 4.5]])
    test_result = np.array([3.5, 4.2])
...
def test_daily_min_string():
    ...
    with pytest.raises(TypeError):
        error_expected = daily_min([['hi', 'there'], ['Mister', 'Rogers']])
